...
> 用途：指导后续本地 Agent 在**现有 Git 仓库**中持续开发 Rem。
...
测试期约定：在首次稳定发布前，不为未发布过的旧 Schema 长期保留兼容分支。若新的领域模型明显更合理，可以在备份 `.gallery` 元数据后直接升级、替换或重建测试数据与本机索引；无论如何都不得借 Schema 调整移动、覆盖或删除媒体原文件。进入稳定发布后再恢复面向已发布版本的兼容迁移承诺。
...
但 Rem 的核心不是 EhViewer：

> E-Hentai 只是可选 Metadata Provider，Rem 本质是本地便携媒体库。
...
**这是完成定义的一部分，不是可选的收尾动作。** 任何一次内容开发、性能优化或缺陷修复，在结束前把"遇到的问题"和"解决措施"写进 `docs/DEV_LOG.md`。

要写的是**过程**，不是结果。CHANGELOG 面向使用者的版本变化；开发记录面向后来的人和 Agent：当时为什么这么判断、试过什么、哪个假设被推翻了、留下了什么约束。只写结果等于把最容易重犯的坑丢掉。

每条记录至少包含：

| 字段 | 要求 |
| --- | --- |
| 问题 | 可观察的现象，以及是谁在什么条件下遇到的 |
| 定位 | 具体文件与行为，不接受"大概是性能问题"这类判断 |
| 根因 | 机制层面的解释；讲不清就是还没定位 |
| 措施 | 改了什么，以及为什么这样改 |
| 验证 | 测试、实测数字或复现步骤；"编译通过"不算验证 |
| 遗留 | 已知限制、待办，以及明确不打算做的事 |

规则：

- **有判断修正就写进去。** 中途被推翻的结论和最终结论同样重要，"我一开始以为是 A，实际是 B，因为 C"正是记录的价值所在。
- **记录证据，不记录功劳。** 引用文件、行号、命令输出和实测数据。
- **不确定的标注为不确定。** 不把估算写成实测。
- **同一个问题只写一条**，后续进展追加到原条目，不要另起一条。
- 用中文写，与本文档一致。
- 记录本身可以单独成一个 `docs(devlog): ...` Commit，也可以并入当次模块的 Commit。
