...
`DirectComicPage` probes visible/composed file or content-URI pages on IO with BitmapFactory bounds and EXIF orientation before starting its image request. Known aspect ratios survive saved-state restoration and constrain both decoding placeholders and final images. Unknown/unsupported headers fall back to normal decoding. This does not enumerate every page. Archive pages publish dimensions from the existing bounds decode through repository/ViewModel to `DecodedComicPage`, without a second preflight read. Initial unknown dimensions still use the existing placeholder. Archive decoding continues to use BitmapFactory's encoded orientation; EXIF normalization there remains outstanding.
