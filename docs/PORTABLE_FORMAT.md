...
外部编辑器负责 revision 和时间戳的协议维护，不能让语言模型凭猜测手改；当前 library-tool 只提交现有 Work 字段的单份 catalog，使用 expected_catalog_sha256 绑定快照，保留未知扩展及人工锁。新建实体、关系和多文档事务不在该工具当前范围。初始化/升级时 identity 最后提交；一般多文档编辑不具备整体原子性，需计划有效中间态、备份及恢复，不承诺一次 rename 解决全部文档一致性。
