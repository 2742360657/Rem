...
- 用数据库 UPDATE 触发器注入故障，断言登记名称也回滚、已清理旧卡恢复、URI 保留，再移除触发器重试成功。最初误用 INSERT 触发器，测试不发生故障，改为实际更新路径后通过。
- Windows / Java 17：305 单测、lint、debug/test APK 与 API 36 四项接入测试通过。没有便携或真实媒体变更；不把 SQLite 事务外推为便携初始化/迁移或真实断电测试。
...
- 测试最初因 DocumentFile 权限预检失败而假通过，加具体错误断言后识别。测试 Context 仅针对合成 Provider 模拟权限，保留真实 Provider 查询/流；修复前确认损坏 catalog 被返回成功，修复后不登记且原坏字节未改。恢复测试需写有效空 catalog，空字节不是有效恢复。
- 305 单测、lint、debug/test APK 与 API 36 十二项接入/SAF 测试通过；补修复后重试成功断言，单项再次通过。无真实库访问。仍需冷接入投影与数据库事务验证，不宣称原真机问题完整关闭。

## 2026-09-19：归档漫画 EXIF 方向归一化

- R07：归档 BitmapFactory 解码不自动应用 EXIF，原页可能横倒或镜像，预留比例也不符合显示方向。复用归档条目访问封装读取 EXIF，尺寸与采样目标交换坐标轴，采样后旋转/镜像再缓存；只有独占的变换前位图被回收，不改源图。
- Windows / Java 17：305 单测、lint、debug/test APK 通过。API 36 先四项实际归档测试通过，再补八方向角点映射测试，九项方向/归档/页面恢复测试通过。JPEG/ZIP 合成红蓝图验证 90° 后 160×80、左右颜色和同来源缓存。
- 每次冷位图解码增加一次 EXIF 条目头部读取，正常从本机归档缓存读取；未知/损坏 EXIF 回退未旋转，取消继续传播。未做本轮全量设备、真机和大归档性能验证。
...
- 不可解析的头部回退原解码路径，错误与取消分开。尚未给归档页接入，首次尺寸探测前的占位仍可能变化，慢 Provider 上增加头部读取的性能待实际测量。
- Windows / Java 17：299 单测、lint、debug/test APK 与 API 36 全部 72 设备测试通过。随后扩充横图/长图，三项 ComicPageLayoutTest 再次通过：用 Coil 拦截器挂起实际像素加载，确认普通竖页、横图、长图在尺寸就绪后和解码完成时高度一致。ComicPageDimensionsTest 验证 EXIF 90 度、流关闭、坏数据、缺失和取消传播。
- 无便携字段、Schema 或真实媒体修改；全量 72 项不冒充新增两个测试后的全量 74 项运行。

## 2026-09-19：漫画单页重试与加载状态隔离

- R07/R11：普通图片页失败提示没有重试入口；归档页 produceState 的键改变不会重置已持有结果，重试期间仍停留在旧错误，来源变化也可能短暂显示旧图。
- 提取实际使用的 DirectComicPage/DecodedComicPage：前者用 painter.restart 重试原请求，后者按身份和重试次数创建新结果状态，明确传播 CancellationException。共用有高度的加载占位，保持此前避免零高度跳页的修复。
- Windows / Java 17：299 单测、lint、debug/test APK 和 API 36 五项设备测试通过。新增三项验证实际缺失文件补回后的原地重试、挂起重试显示进度后成功、身份切换清旧图及销毁取消；已有两项 ReaderPosition 实际组件测试回归通过。测试首次编译因进度匹配器缺参数失败，修正后通过。
- 无便携或真实媒体修改。尚未解决解码前宽高比和加载后高度变化，不能称已达到完整 EhViewer 阅读体验；取消同步 Provider 读取的工程边界不变。

## 2026-09-19：隔离归档漫画位图缓存身份

- R07/R11 检查阅读解码时发现缓存键缺 Library 和归档大小，合并 Edition 还错误沿用外层 Work 时间戳。新增真实 PNG/ZIP + 测试 SAF Provider 场景，先复现两项失败：蓝页返回旧红页。
- 使用结构化缓存键，纳入 Library、路径、大小、时间戳、条目及解码尺寸；合并来源先解析实际容器版本，并把同一版本用于归档读取。保持相同来源的缓存命中。
- Windows / Java 17：299 单测、lint、debug/test APK 通过；API 36 六项归档/ReaderPosition/OfflinePreview 通过。新增缓存命中断言后，三项归档测试再次通过，包括跨库、大小变化、合并容器变化。未重跑全部设备测试，未访问真实 Library。
- 尚不能承诺元数据完全不变的原地字节替换自动失效；解码前宽高比及连续阅读布局稳定性仍是后续工作。

## 2026-09-19：系统相册筛选、定位与刷新恢复

- R03/R07：新增媒体类型和来源组合筛选，保留查询排序；按筛选保存网格位置，复用序号跳转对话框。刷新中的空结果不清选择，加载时禁止导入，完成刷新后按可访问 URI 清理。
- 实际 Compose 测试发现只换 SaveableStateProvider 的键仍会沿用活跃网格状态；外层加入 key 隔离组合实例后，视频→照片→视频恢复通过。另一新增测试起初误匹配来源卡片文字，改为匹配带 Selected 语义的筛选按钮。
- Windows / Java 17：testDebugUnitTest、lintDebug、assembleDebug、assembleDebugAndroidTest、connectedDebugAndroidTest 通过；299 单测、API 36 全部 63 设备测试。新增两项各用 150 项合成数据，覆盖加载空快照、重建、组合筛选和访问范围收缩。
- 无便携数据变化、无真实媒体访问；真机已移除，真实相册导入、长时间手势与大库性能仍未验收。

## 2026-09-19：分组书架按内容定位

- R04/R07：分组书架包含新建说明及两类标题，原始 LazyColumn 索引不是内容序号。ListPositionButton 新增可选有序 itemIndices 映射；书架只计手动/派生分组，列表状态在空内容分支前持有。
- API 36 八项共享定位设备回归通过。新增 150 手动分组加 1 派生分组，验证第 100 项正确、内容暂空期间状态重建后恢复锚点、末项展示派生组。Windows 299 单测、lint、debug/test APK 构建通过；未重跑上轮 60 项全量设备测试。
...
- 新增 PageLoadState/rememberPageLoad 与加载反馈：错误或空页可原地重试，加载时可取消返回，协程取消正常传播。API 36 用一次失败后成功及挂起任务取消两场景通过；完整单测/lint/debug/test APK 构建通过。第一次验证命令自动审批超时且未启动，按工具允许重试后成功；无遗留审批阻塞。
...
- 后续 R07 审查发现 VideoViewer 在退出时用 `position >= duration - 5000` 判定完成，短片刚打开也可能满足；修为已播放后收到 STATE_ENDED 才设完成，恢复旧完成标记，完成作品重播从零开始。旧进度尚未加载时不保存零进度。两项纯状态回归及 API 36 ExoPlayer 一秒合成 WAV 实播验证通过；WAV 验证事件语义，不代表视频解码或剧集连播验收。
- R08 两个模块提交后完整 API 36 设备回归 36 项通过。视频修正后全套单测、lint、debug/test APK 构建通过，并单独通过新增播放器设备测试。
