...
- 补全阶段仍以读取文件前取得的 `MediaItem` 生成整行更新；若同一时刻用户编辑该项目，可能暂时覆盖较新的 SQLite 投影。便携元数据不会丢失，但下一轮应改为提交补全结果前重新读取当前行并做字段合并，且不要在媒体 I/O 期间持有便携写锁。
- 漫画页已支持点击隐藏控件与放大时锁住当前页，但尚未复用经手势测试的统一 `Zoomable`；Series 也尚未在一话结束后自动衔接下一话。
